...
cell_seurat_data <- readRDS("/tgen_labs/banovich/BCTCSF/13384_GBMHGG_Xenium/Seurat/spatial_clustered_NN30_PC50_Seurat.rds")
...
tma_paths <- list("TMA1_0069754" = "/tgen_labs/banovich/xenium_run_folders/20251003__191226__COMBO_NB-TGenTMA_CB-hBrainTMA1/output-XETG00048__0069754__CB_hBrainTMA1__20251003__191242/transcripts.parquet")

# Count matrix
mat_tma1 <- as.matrix(cell_seurat_data@assays$RNA$counts)

# Coordinates
coords_tma1 <- cell_seurat_data@meta.data %>%
  select(x_centroid, y_centroid) %>%
  rename_with(~ "x", .cols = "x_centroid") %>%
  rename_with(~ "y", .cols = "y_centroid")

# Xenium data
tx_tma1 <- read_parquet(tma_paths[["TMA1_0069754"]])
...
res <- denoist(mat = mat_tma1,
               ...
               out_dir = "/scratch/hnatri/DenoIST/GBMHGG_TMA_outputs/",
               output_name = "TMA1_0069754")
