...
cells <- seurat_data@meta.data[,c("cell_id", "Sample", "predicted.id", "x_centroid", "y_centroid")]
...
keep_cts <- table(cells$final_CT) %>% as.data.frame() %>%
  filter(Freq > 1000)

#cells <- cells %>% filter(!final_CT == "L9")
cells <- cells %>% filter(final_CT == keep_cts$Var1)
...
write.table(proximity_compiled,
            "/home/hnatri/13384_GBMHGG_SPP1_Xenium/tumor_proximity_compiled.tsv",
...
write.table(enrichment_scores_compiled,
            "/home/hnatri/13384_GBMHGG_SPP1_Xenium/enrichment_tumor_scores_compiled.tsv",
            quote = F, row.names = F, sep = "\t")
